...
router.post("/reset-password", resetPassValidation, async (req, res) => {
  ...
  if (user && user._id) {
    ...
    return res.json({
      status: "success",
      message:
        "if your email exist in our db then a password reset email wil be sent to you in few seconds",
    });
  }
...
router.patch("/reset-password", updatePassValidation, async (req, res) => {
  ...
  if (getPin._id) {
    ...
    if (today > expDate) {
      return res.json({
        status: "error",
        message: "The pin is invalid or expired",
      });
    }
...
router.delete("/logout", userAuthorization, async (req, res) => {
  ...
  res.json({
    status: "error",
    message: "cant logged out!!!!!!try again later",
  });
});
